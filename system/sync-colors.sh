...
# === SUPERFILE ===
echo "Updating superfile..."
mkdir -p "$CONFIG_DIR/superfile/theme"
# Secondary text: soft blend of fg over bg (readable, less harsh than pure white)
SPF_FG_MUTED="#d7dadf"
cat > "$CONFIG_DIR/superfile/theme/cyberdream.toml" << SUPERFILE_EOF
##############################################
#                                            #
#              Cyberdream Theme              #
#                                            #
...
# Palette source: ~/.config/colorscheme/cyberdream.json
# Regenerated by: ~/.config/colorscheme/sync-colors.sh
# Docs: https://superfile.dev/configure/custom-theme/

###############################################################################
#                           Code Syntax Highlighting                          #
###############################################################################

# Closest neon-dark chroma style to cyberdream accents
# (styles: https://github.com/alecthomas/chroma/tree/master/styles)
code_syntax_highlight = "tokyonight-night"
...
#-- Full Screen
full_screen_fg = "${SPF_FG_MUTED}"
full_screen_bg = "${BG}"

#-- Gradient (title bar accent — blue → purple)
# Note: only two colors supported
gradient_color = ["${BLUE}", "${MAGENTA}"]

# Fallback directory icon color (empty = superfile default)
directory_icon_color = "${CYAN}"

#-- File Panel
file_panel_fg = "${SPF_FG_MUTED}"
file_panel_bg = "${BG}"
file_panel_border = "${BG_ALT}"
file_panel_border_active = "${CYAN}"
file_panel_top_directory_icon = "${CYAN}"
file_panel_top_path = "${BLUE}"
file_panel_item_selected_fg = "${ORANGE}"
file_panel_item_selected_bg = "${BG_ALT}"

#-- Footer
footer_fg = "${SPF_FG_MUTED}"
footer_bg = "${BG}"
footer_border = "${BG_ALT}"
...
sidebar_item_selected_bg = "${BG_ALT}"
sidebar_divider = "${BG_ALT}"

#-- Modals
modal_fg = "${SPF_FG_MUTED}"
modal_bg = "${BG}"
modal_border_active = "${PINK}"
modal_cancel_fg = "${BG}"
modal_cancel_bg = "${RED}"
modal_confirm_fg = "${BG}"
...
# Ensure config points at cyberdream (idempotent)
if [[ -f "$CONFIG_DIR/superfile/config.toml" ]]; then
    sed -i 's/^theme = .*/theme = "cyberdream"/' "$CONFIG_DIR/superfile/config.toml"
fi
...
echo "  - Superfile: restart spf to load theme"
